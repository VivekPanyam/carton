...
async fn main() {
    let runner_info = RunnerInfo {
        runner_name: "xgboost".into(),
        required_framework_version: semver::VersionReq::parse("=1.7").unwrap(),
...
async def main():
    info = await carton.get_model_info("https://carton.pub/cartonml/basic_test")
...
const info = await get_model_info("https://carton.pub/cartonml/basic_test");
...
async fn main() {
    ...
    let data = Carton::get_model_info(
        "https://carton.pub/cartonml/basic_test",
    )
    .await
    .unwrap();
